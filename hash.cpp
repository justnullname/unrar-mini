...
void HashValue::Init(HASH_TYPE TypeVal)
{
  HashValue::Type=TypeVal;
...
void DataHash::Init(HASH_TYPE Type,[[maybe_unused]] uint MaxThreads)
{
  if (blake2ctx==NULL)
...
uint DataHash::gfMulCRC(uint A, uint B)
{
  ...
  const uint POLY=0x04c11db7; // Lower 32 bits of 0x104c11db7
...
bool DataHash::Cmp(HashValue *CmpValue, [[maybe_unused]] byte *Key)
{
  HashValue Final;
  Result(&Final);
#ifndef RAR_NOCRYPT
