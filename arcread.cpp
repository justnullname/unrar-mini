    ...
    case HEAD_SERVICE:
      {
        ...
        if (FileBlock)
        {
          ...
          if ((hd->Flags & LHD_UNICODE)!=0)
          {
            ...
            size_t Length=strlen(NameData.data());
            Length++;
            if (ReadNameSize>Length)
              NameCoder.Decode(NameData.data(),ReadNameSize,
                               (byte *)&NameData[Length],
                               ReadNameSize-Length,hd->FileName);
          }

          if (hd->FileName.empty())
            ArcCharToWide(NameData.data(),hd->FileName,ACTW_OEM);
        ...
        {
          CharToWide(NameData.data(),hd->FileName);
    ...
    case HEAD_SERVICE:
      {
        ...
        *(BaseBlock *)hd=ShortBlock;
        ...
        UtfToWide(name_utf8.data(),hd->FileName);
  ...
  while (Raw->DataLeft()>=2)
  {
    ...
    if (bb->HeaderType==HEAD_FILE || bb->HeaderType==HEAD_SERVICE)
    {
      ...
      switch(FieldType)
      {
        ...
        case FHEXTRA_CRYPT:
          {
            FileHeader *hd_crypt=(FileHeader *)bb;
            uint EncVersion=(uint)Raw->GetV();
            if (EncVersion>CRYPT_VERSION)
            {
              UnkEncVerMsg(hd_crypt->FileName,L"x" + std::to_wstring(EncVersion));
              hd_crypt->CryptMethod=CRYPT_UNKNOWN;
            }
            else
            {
              uint Flags=(uint)Raw->GetV();
              hd_crypt->Lg2Count=Raw->Get1();
              if (hd_crypt->Lg2Count>CRYPT5_KDF_LG2_COUNT_MAX)
              {
                UnkEncVerMsg(hd_crypt->FileName,L"xc" + std::to_wstring(hd_crypt->Lg2Count));
                hd_crypt->CryptMethod=CRYPT_UNKNOWN;
              }
              else
              {
                hd_crypt->UsePswCheck=(Flags & FHEXTRA_CRYPT_PSWCHECK)!=0;
                hd_crypt->UseHashKey=(Flags & FHEXTRA_CRYPT_HASHMAC)!=0;

                Raw->GetB(hd_crypt->Salt,SIZE_SALT50);
                Raw->GetB(hd_crypt->InitV,SIZE_INITV);
                if (hd_crypt->UsePswCheck)
                {
                  Raw->GetB(hd_crypt->PswCheck,SIZE_PSWCHECK);
                  ...
                  byte Digest[SHA256_DIGEST_SIZE];
                  sha256_get(hd_crypt->PswCheck, SIZE_PSWCHECK, Digest);

                  hd_crypt->UsePswCheck=memcmp(csum,Digest,SIZE_PSWCHECK_CSUM)==0;

                  // RAR 5.21 and earlier set PswCheck field in service records to 0
                  // even if UsePswCheck was present.
                  if (bb->HeaderType==HEAD_SERVICE && memcmp(hd_crypt->PswCheck,"\0\0\0\0\0\0\0\0",SIZE_PSWCHECK)==0)
                    hd_crypt->UsePswCheck=0;
                }
                hd_crypt->SaltSet=true;
                hd_crypt->CryptMethod=CRYPT_RAR50;
                hd_crypt->Encrypted=true;
              }
            }
          }
          break;
        ...
        case FHEXTRA_HASH:
          {
            FileHeader *hd_extra=(FileHeader *)bb;
            uint Type=(uint)Raw->GetV();
            if (Type==FHEXTRA_HASH_BLAKE2)
            {
              hd_extra->FileHash.Type=HASH_BLAKE2;
              Raw->GetB(hd_extra->FileHash.Digest,BLAKE2_DIGEST_SIZE);
            }
          }
          break;
        case FHEXTRA_HTIME:
...
size_t Archive::ReadHeader14()
{
  ...
  else
  {
    ...
    std::string name_utf8(NameSize,0);
    Raw.GetB((byte *)&name_utf8[0],NameSize);
    std::string NameA;
    OemToExt(name_utf8,NameA);
