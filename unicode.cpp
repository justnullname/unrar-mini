...
void WideToUtf(const std::wstring &Src,std::string &Dest)
{
  for (size_t I=0;I<Src.size() && Src[I]!=0;)
  {
    ...
    if (c<0x80)
      ...
      if (c<0x800)
      {
        Dest.push_back((char)(0xc0|(c>>6)));
        Dest.push_back((char)(0x80|(c&0x3f)));
      }
      else
      {
        ...
        if (c<0x10000)
        {
          Dest.push_back((char)(0xe0|(c>>12)));
          Dest.push_back((char)(0x80|((c>>6)&0x3f)));
          Dest.push_back((char)(0x80|(c&0x3f)));
        }
        else
          if (c < 0x200000)
          {
            Dest.push_back((char)(0xf0|(c>>18)));
            Dest.push_back((char)(0x80|((c>>12)&0x3f)));
            Dest.push_back((char)(0x80|((c>>6)&0x3f)));
            Dest.push_back((char)(0x80|(c&0x3f)));
          }
      }
...
bool UtfToWide(const char *Src,std::wstring &Dest)
{
  ...
  while (*Src!=0)
  {
    ...
    if (d>0xffff)
    {
      ...
      {
        Dest.push_back( (wchar)(((d-0x10000)>>10)+0xd800) );
        Dest.push_back( (wchar)((d&0x3ff)+0xdc00) );
      }
      else
        Dest.push_back( (wchar)d );
    }
    else
      Dest.push_back( (wchar)d );
  }
...
void SupportDBCS::Init()
{
  ...
  for (uint I=0;I<ASIZE(IsLeadByte);I++)
    IsLeadByte[I]=IsDBCSLeadByte((BYTE)I)!=0;
}
