...
void ModelPPM::RestartModelRare()
{
  ...
  if (FoundState == NULL)
    return;
  for (RunLength=InitRL, PrevSuccess=(byte)0, i=0;i < 256;i++) 
  {
...
void ModelPPM::StartModelRare(int MaxOrderVal)
{
  int i, k, m ,Step;
  EscCount=1;
/*
...
void RARPPM_CONTEXT::rescale(ModelPPM *Model)
{
  ...
  U.SummFreq = (p->Freq=static_cast<byte>((p->Freq+Adder) >> 1));
  do 
  {
    EscFreq -= (++p)->Freq;
    U.SummFreq += (p->Freq=static_cast<byte>((p->Freq+Adder) >> 1));
  ...
  if (p->Freq == 0) 
  {
    ...
    EscFreq += i;
    if ((NumStats = (ushort)(NumStats - i)) == 1) 
    {
  ...
  U.SummFreq = (ushort)(U.SummFreq + (EscFreq -= (EscFreq >> 1)));
...
inline void RARPPM_CONTEXT::decodeBinSymbol(ModelPPM *Model)
{
  ...
  {
    Model->FoundState=&rs;
    rs.Freq = (byte)(rs.Freq + (rs.Freq < 128));
...
{
  (Model->FoundState=p)->Freq = (byte)(p->Freq + 4);              
  U.SummFreq += 4;
  if (p[0].Freq > p[-1].Freq) 
  {
    _PPMD_SWAP(p[0],p[-1]);                   
...
inline bool RARPPM_CONTEXT::decodeSymbol1(ModelPPM *Model)
{
  ...
  if (count < (HiCnt=p->Freq)) 
  {
    ...
    (Model->FoundState=p)->Freq=static_cast<byte>(HiCnt += 4);
    U.SummFreq += 4;
    if (HiCnt > MAX_FREQ)
      rescale(Model);
    Model->Coder.SubRange.LowCount=0;
...
inline void RARPPM_CONTEXT::update2(ModelPPM *Model,RARPPM_STATE* p)
{
  ...
  if (p->Freq > MAX_FREQ)                 
    rescale(Model);
  Model->EscCount=(ushort)(Model->EscCount + 1);
...
{
  int MaxOrderVal=UnpackRead->GetChar();
  bool Reset=(MaxOrderVal & 0x20)!=0;
  ...
  if (Reset)
  {
    MaxOrderVal=(MaxOrderVal & 0x1f)+1;
    if (MaxOrderVal>16)
      MaxOrderVal=16+(MaxOrderVal-16)*3;
    if (MaxOrderVal==1)
    {
      SubAlloc.StopSubAllocator();
      return(false);
