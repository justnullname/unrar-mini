/*
 * unrar-mini: Modernized UnRAR extraction engine.
 * Based on UnRAR source by Alexander Roshal.
 * 
 * License: UnRAR License (see LICENSE file for details).
 * This code may be used for extraction only.
 */
void Unpack::Unpack5(bool Solid)
{
  ...
  while (true)
  {
    ...
    {
      if (Fragmented)
        FragWindow[UnpPtr++]=static_cast<byte>(MainSlot);
      else
        Window[UnpPtr++]=static_cast<byte>(MainSlot);
      continue;
    }
    if (MainSlot>=262)
    {
...
bool Unpack::ReadFilter(BitInput &inp,UnpackFilter &Filter)
{
  ...
  Filter.Type=static_cast<byte>(inp.fgetbits()>>13);
  inp.faddbits(3);

  if (Filter.Type==FILTER_DELTA)
  {
    Filter.Channels=static_cast<byte>((inp.fgetbits()>>11)+1);
    inp.faddbits(5);
  }
  ...
  for (size_t I=0;I<Filters.size();I++)
  {
    ...
    if (WrapDown(BlockStart-WrittenBorder)<WriteSizeLeft)
    {
      ...
      else
      {
        ...
        {
          UnpackFilter *flt_j=&Filters[J];
          if (flt_j->Type!=FILTER_NONE)
            flt_j->NextWindow=false;
        }
...
bool Unpack::ReadBlockHeader(BitInput &inp,UnpackBlockHeader &Header)
{
  ...
  byte BlockFlags=static_cast<byte>(inp.fgetbits()>>8);
  inp.faddbits(8);
  ...
  byte SavedCheckSum=static_cast<byte>(inp.fgetbits()>>8);
  inp.faddbits(8);

  int BlockSize=0;
  for (uint I=0;I<ByteCount;I++)
  {
    BlockSize+=static_cast<int>((inp.fgetbits()>>8)<<(I*8));
    inp.addbits(8);
  }

  Header.BlockSize=BlockSize;
  byte CheckSum=static_cast<byte>(0x5a^BlockFlags^BlockSize^(BlockSize>>8)^(BlockSize>>16));
...
bool Unpack::ReadTables(BitInput &inp,UnpackBlockHeader &Header,UnpackBlockTables &Tables)
{
  ...
  for (uint I=0;I<BC;I++)
  {
    uint Length=static_cast<byte>(inp.fgetbits() >> 12);
    inp.faddbits(4);
    if (Length==15)
    {
      uint ZeroCount=static_cast<byte>(inp.fgetbits() >> 12);
      inp.faddbits(4);
      if (ZeroCount==0)
        BitLength[I]=15;
      else
      ...
      BitLength[I]=static_cast<byte>(Length);
  }
  ...
  for (uint I=0;I<TableSize;)
  {
    ...
    if (Number<16)
    {
      Table[I]=static_cast<byte>(Number);
      I++;
    }
    else
      if (Number<18)
